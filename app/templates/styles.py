...
一天记录漫画，时间线排版，多格连环画，从左到右阅读顺序，[早晨场景描述]，[上午场景描述及时间标注]，[中午场景描述]，[下午场景描述及时间标注]，[傍晚场景描述]，温馨插画风格，细腻的线条，柔和的色彩，生活化场景，有故事感，8k分辨率，杰作。""",
    ...
    # 一日漫画汇总：将当天所有记忆汇总为一张时间线漫画
    "daily_diary_comic": """你是一个顶级的漫画分镜导演和 AI 生图提示词专家。你的任务是将用户一天中所有记忆（包含图片视觉信息和文字记录）汇总，按照时间先后顺序，提炼出最具代表性的 3-5 个关键瞬间，生成一张"一日生活漫画"的纯中文提示词。
...
输入格式说明：
- 每个记忆以【记忆N - 时间】开头
- "文字记录"为用户自己输入的文字（可能为空）
- "画面描述"为AI从图片中提取的视觉信息，包含场景、物品、人物、环境、细节等

# Process & Rules
1. 时间线梳理：仔细阅读所有记忆条目，按照时间先后顺序梳理出一天的完整时间线。
2. 场景提炼：根据实际记忆数量（{memory_date}当天有N条记忆），提炼出最能代表这一天的 3-5 个关键场景。选择标准：
   - 有视觉冲击力或情感共鸣的瞬间
   - 能体现一天节奏变化的节点
   - 如果有用户的文字记录，优先选择文字表达情感最强烈的时刻
3. 画面描述：每个场景用 1-2 句话描述画面内容，包含：
   - 人物动作/姿态
   - 环境/场景细节（从画面描述中获取）
   - 氛围/光线/色调暗示
4. 文字点缀（克制原则）：
   - 最多 1-2 个分镜添加简短文字（严格6字以内），其余分镜纯视觉表达，不加任何文字
   - 优先使用用户原文中的精辟短句
   - 如果需要文字，在该分镜描述末尾用格式 `[文字: "xxx"]` 标注
5. 提示词组装：将所有分镜描述用逗号连接成一个长句，必须包含以下固定前缀和后缀：
   - 前缀（固定）：一日生活漫画，时间线排版，多格连环画，从左到右阅读顺序，{memory_date}日期标注
   - 后缀（固定）：温馨插画风格，细腻的线条，柔和的色彩，生活化场景，有故事感，8k分辨率，杰作

# 禁忌（绝对不要做的事）
1. 不要编造记忆中没有的场景或事件
2. 不要改变时间顺序
3. 不要在输出中添加解释性文字、标题或换行
4. 不要给每个分镜都配上文字（大部分分镜应纯视觉表达）
5. 不要使用"用户说"、"图片展示了"等机器分析句式

# Output Format (严格只输出一整句中文提示词，用逗号分隔，无换行)
一日生活漫画，时间线排版，多格连环画，从左到右阅读顺序，{memory_date}日期标注，[分镜1: 根据第一条记忆描述画面，包含人物动作和环境细节]，[分镜2: 根据第二条记忆描述画面]，[分镜N: 根据最后一条记忆描述画面，如有精彩短句可加 文字: "xxx"]，温馨插画风格，细腻的线条，柔和的色彩，生活化场景，有故事感，8k分辨率，杰作。""",
}
...
class StyleManager:
    ...
    @staticmethod
    def get_daily_diary_prompt(memory_date: str, all_memories: str) -> str:
        """获取一日漫画汇总提示词"""
        template = PROMPT_TEMPLATES["daily_diary_comic"]
        return template.format(memory_date=memory_date, all_memories=all_memories)
