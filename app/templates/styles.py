...
# Process & Rules
1. 时间线梳理：仔细阅读所有记忆条目，按照拍摄时间/记录时间的先后顺序梳理出一天的完整时间线。
2. 场景提炼：从时间线中提取 3-5 个最能代表这一天的关键场景。选择标准：
   - 有视觉冲击力或情感共鸣的瞬间
   - 能体现一天节奏变化的节点（如：早晨开始→工作/学习→午间休息→下午活动→晚间收尾）
   - 如果有用户的文字记录，优先选择文字表达情感最强烈的时刻
3. 画面描述：每个场景用 1-2 句话描述画面内容，包含：
   - 人物动作/姿态
   - 环境/场景细节（从VLM提取的视觉信息中获取）
   - 氛围/光线/色调暗示
4. 文字点缀（克制原则）：最多 1-2 个分镜添加简短文字（6字以内），其余纯视觉表达。
   - 优先使用用户原文中的精辟短句
5. 提示词组装：必须严格遵守以下框架：
   - 布局词：一日生活漫画，时间线排版，多格连环画，从左到右阅读顺序，日期标注
   - 风格词：温馨插画风格，细腻的线条，柔和的色彩，生活化场景，有故事感

# Output Format (严格只输出中文提示词组合)
一日生活漫画，时间线排版，多格连环画，从左到右阅读顺序，{memory_date}日期标注，[早晨场景描述]，[上午场景描述]，[中午/下午场景描述]，[傍晚/晚间场景描述]，温馨插画风格，细腻的线条，柔和的色彩，生活化场景，有故事感，8k分辨率，杰作。""",
}
